...
impl File {
        ...
        self.name = new_path
            ...
            .ok_or_else(|| {
                failure::err_msg(format!(
                    "Couldn't get new filename in path '{}'",
    ...
    pub fn is_readable(&self) -> HResult<bool> {
        ...
        let file_user = get_user_by_uid(meta.uid())
            .ok_or_else(|| failure::err_msg(format!("Couldn't get user with id '{}'", meta.uid())))?
            .name()
            .to_string_lossy()
            .to_string();
        ...
        let file_group = get_group_by_gid(meta.gid())
            .ok_or_else(|| {
                failure::err_msg(format!("Couldn't get group with id '{}'", meta.uid()))
            })?
            .name()
            .to_string_lossy()
            .to_string();
