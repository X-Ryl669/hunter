        ...
        let run_preview = Box::new(
            move |auto,
                ...
                loop {
                    ...
                    let mut stdout = BufReader::new(previewer.stdout.take().ok_or_else(|| {
                        failure::err_msg("Couldn't connect to stdout for hunter-media previewer")
                    })?);
                    let mut stdin = previewer.stdin.take().ok_or_else(|| {
                        failure::err_msg("Couldn't connect to stdin for hunter-media previewer")
                    })?;
                    ...
                    *cprocess.lock() = Some(previewer);
                    ...
                    loop {
                        ...
                        if let Ok(Some(code)) = cprocess
                            ...
                            .as_mut()
                            .ok_or_else(|| {
                                failure::err_msg("Couldn't acquire lock on hunter-media thread")
                            })?
                            .try_wait()
                        {
                            if code.success() {
