    ...
    fn read_proc(&mut self) -> HResult<()> {
        ...
        std::thread::spawn(move || -> HResult<()> {
            let stdout = handle
                ...
                .stdout
                .take()
                .ok_or_else(|| failure::err_msg("Couldn't connect to stdout for cmd thread"))?;
...
impl ProcView {
    ...
    fn show_output(&mut self) -> HResult<()> {
        ...
        let selected_proc = match self.get_listview_mut().selected_proc() {
            Some(p) => p,
            None => return Ok(()),
        };

        let output = selected_proc.output.lock().clone();
